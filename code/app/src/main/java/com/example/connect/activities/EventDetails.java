...
public class EventDetails extends AppCompatActivity {
        ...
        db.collection("events")
                ...
                .addOnSuccessListener(documentSnapshot -> {
                    if (documentSnapshot.exists()) {
                        ...
                        // Fetch actual waiting list count from waiting_lists collection
                        db.collection("waiting_lists")
                                .document(eventId)
                                .get()
                                .addOnSuccessListener(wlSnapshot -> {
                                    long count = 0;
                                    if (wlSnapshot.exists()) {
                                        List<String> entries = (List<String>) wlSnapshot.get("entries");
                                        if (entries != null) {
                                            count = entries.size();
                                        }
                                    }

                                    String waitingListText = count + " Joined";

                                    // Display the details
                                    displayEventDetails(eventName, organizationName, dateTime, location, price,
                                            registrationWindow, waitingListText);
                                })
                                .addOnFailureListener(e -> {
                                    // Fallback if waiting list fetch fails
                                    String waitingListText = "0 Joined";
                                    displayEventDetails(eventName, organizationName, dateTime, location, price,
                                            registrationWindow, waitingListText);
                                });
    ...
    private void displayEventDetails(String eventName, String organizationName,
        ...
        // Use tvOrgName for description body as per new layout mapping
        tvOrgName.setText(description != null ? description : "No description available");
        ...
        // Update waiting list count text style
        tvWaitingList.setText(waitingListCount);

        // Parse and set Date Badge
        if (dateTime != null) {
            try {
                // Expected format: "hh:mm a, MMM dd, yyyy"
                String[] parts = dateTime.split(", ");
                if (parts.length >= 2) {
                    String datePart = parts[1]; // "MMM dd"
                    ...
                    if (dateComponents.length >= 2) {
                        ...
                        TextView tvDateMonth = findViewById(R.id.tv_date_month);

                        if (tvDateDay != null)
                            tvDateDay.setText(day);
                        if (tvDateMonth != null)
                            tvDateMonth.setText(month.toUpperCase());
                    }
                }
            } catch (Exception e) {
