...
- 📅 **QR Code Scanning:** _Entrants can scan QR promotional code to view details about the event and also join the waiting list directly in the app_
- 👥 **Firebase Integration:** _Utilize Firebase for storing event details, attendee lists, and real-time check-in status updates and more_
...
- 📊 **Image Upload:** _Allow event organizers and users to upload event poster and profile pictures_
- 🗺️ **Geolocation Verification:** _Users are verified for events through geolocation when joining the waitlist (withing 20km radius) - Dependent on if the event requires its_
